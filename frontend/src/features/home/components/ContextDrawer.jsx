...
export function ContextDrawer({ open, onClose, selectedItem, summary, research, dataHealth, governance }) {
  return (
    <>
      <AnimatePresence>
        {open ? (
          <motion.div animate={{ opacity: 1 }} className="context-drawer__backdrop" exit={{ opacity: 0 }} initial={{ opacity: 0 }} onMouseDown={onClose}>
            ...
            >
              <header><div><span className="context-drawer__eyebrow">Selected context</span><h2>{selectedItem?.title ?? "Overview context"}</h2>{selectedItem?.sourceDomain ? <p className="context-drawer__source">{selectedItem.category}</p> : null}</div><button aria-label="Close contextual drawer" className="app-icon-button" onClick={onClose} type="button"><AppIcon name="close" /></button></header>
