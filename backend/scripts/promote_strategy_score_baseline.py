...
from app.strategy.scoring.score_baseline import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify and freeze the current swing scoring baseline.")
    parser.add_argument("--tests-passed", action="store_true")
    parser.add_argument("--frontend-build-passed", action="store_true")
    args = parser.parse_args()

    if not git_ignored("data/reports/strategy_score_baseline_promotion.json"):
    ...
    report_path = REPO_ROOT / "data" / "reports" / "strategy_score_baseline_promotion.json"
    document_path = REPO_ROOT / "docs" / "strategy-score-baseline-promotion.md"
